...
console.log(fenomenler[7].profile)
...
function indekseGoreFenomen(obj,x) {

  return (`${x}. indekste bulunan fenomen: ${fenomenler[x].profile}`)
}
...
function fenomenSil(fenomenler,x) {
  let newArr = [...fenomenler];
  newArr.splice(x,1);
  return newArr;
}
...
function enFenomenler(fenomenler) {
  ...
  for(let i = 0; i < fenomenler.length; i++){
    if(fenomenler[i].followers > 100000000){
      newArr.push(fenomenler[i].profile);
    }
  }
  return newArr
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler,platform){
  ...
  let enFenomen = '';
  for(let i = 0; i < fenomenler.length; i++){
    if(fenomenler[i].platform == platform && fenomenler[i].posts != 'NA' && fenomenler[i].posts > maxPost){
